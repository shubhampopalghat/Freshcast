...
const AQICard: React.FC = () => {
  ...
  return (
    <div className=" bg-card-bg rounded-3xl p-4 w-full animate-fade-in ">
      ...
      <div ref={gaugeRef} className="gauge-container relative ">
        {/* Semi-circular gauge */}
        <div className="gauge-bg h-[110px] w-full relative">
          ...
          {/* Gauge pointer */}
          <div className="gauge-pointer absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-[95px] bg-white origin-bottom transition-transform duration-1000 bg-zinc-200"></div>
          ...
          <div className="absolute bottom-0 w-full flex justify-between px-4 text-xs text-text-secondary">
            <span className="left-7 bottom-2 relative">Good</span>
            <span className="ml-auto relative right-7 bottom-2">Poor</span>
          </div>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="bg-tertiary/40 p-3 rounded-lg">
          <p className="ml-3 text-xs text-text-secondary">PM2.5</p>
          <p className="ml-3 text-lg font-semibold">{airQualityData.list[0].components.pm2_5.toFixed(1)} µg/m³</p>
        </div>
        <div className="bg-tertiary/40 p-3 rounded-lg">
          <p className="ml-3 text-xs text-text-secondary">PM10</p>
          <p className="ml-3 text-lg font-semibold">{airQualityData.list[0].components.pm10.toFixed(1)} µg/m³</p>
        </div>
      </div>
    </div>
  );
